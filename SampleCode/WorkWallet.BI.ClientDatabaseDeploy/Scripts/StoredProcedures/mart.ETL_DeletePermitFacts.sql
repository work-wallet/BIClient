...
BEGIN
    ...
    DELETE mart.PermitChecklistAnswerFact2
    FROM mart.PermitChecklistAnswerFact2 AS d
    INNER JOIN mart.Permit AS p ON d.Permit_key = p.Permit_key
    INNER JOIN @permitToWorkTable AS a ON p.PermitId = a.PermitToWorkId;

    PRINT 'DELETE mart.PermitChecklistAnswerFact2, number of rows = ' + CAST(@@ROWCOUNT AS varchar);
